...
static irqreturn_t gpio_irq_handler(int irq, void *dev_id)
{
	...
	tmp[0] = gpio_get_value(588) + '0';

    switch(tmp[0]) {
		case '0':
			gpio_set_value(592, 0);
			break;
		case '1':
			gpio_set_value(592, 1);
			break;
		default:
...
static int __init ModuleInit(void) {
	...
	/* GPIO 21 init */
	if(gpio_request(592, "rpi-gpio-21")) {
		printk("Can not allocate GPIO 21\n");
		goto AddError;
	}

	/* Set GPIO 21 direction */
	if(gpio_direction_output(592, 0)) {
		printk("Can not set GPIO 21 to output!\n");
		goto Gpio4Error;
	}

	/* GPIO 17 init */
	if(gpio_request(588, "rpi-gpio-17")) {
		printk("Can not allocate GPIO 17\n");
		goto Gpio4Error;
	}

	/* Set GPIO 17 direction */
	if(gpio_direction_input(588)) {
		printk("Can not set GPIO 17 to input!\n");
		goto Gpio17Error;
	}

    /* Setup the interrupt */
	irq_number = gpio_to_irq(588);
